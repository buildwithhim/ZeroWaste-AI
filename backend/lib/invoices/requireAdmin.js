 ...
 * holds the admin token, not who they are. It exists so that every invoice
 * route has a single, explicit place where access is decided, and so no
 * employee-facing route can reach invoice data by accident.
 *
 * Replace `verify` with the real identity check when SSO lands — the route
 * layer should not need to change.
 */
...
function requireAdmin(req, res, next) {
  if (!tokenMatches(presentedToken(req))) {
    return res.status(403).json({ error: "Administrator access is required for invoice data" });
  }
