...
export default function SustainabilityScore() {
  ...
  const answeredFeedback = lunchBookings.map((booking) => feedbackByBooking.get(booking.id)).filter(Boolean);
  const finishedMeals = answeredFeedback.filter((item) => item?.response === "Finished meal").length;
  const leftoverMeals = answeredFeedback.filter((item) => item?.response === "Left some food").length;
